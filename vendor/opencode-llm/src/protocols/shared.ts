 ...
 * `LLMError`.
 *
 * A proxy or gateway that swallows a backend error and replies `HTTP 200`
 * with a JSON error body (or an empty body) decodes to zero SSE frames — the
 * decoder treats non-`data:` lines as fields and drops them. Those responses
 * would otherwise surface downstream as a silently empty completion. Detect
 * the empty decode (raw body present but no frame ever passed the filter) and
 * fail with the raw body so the caller can surface the real error.
 */
export const sseFraming = (bytes: Stream.Stream<Uint8Array, LLMError>): Stream.Stream<string, LLMError> =>
  Stream.suspend(() => {
    const state = { sawFrame: false, raw: "" }
    const MAX_RAW_BYTES = 8 * 1024
    return bytes.pipe(
      Stream.decodeText(),
      Stream.tap((text) =>
        Effect.sync(() => {
          if (state.raw.length < MAX_RAW_BYTES) state.raw = (state.raw + text).slice(0, MAX_RAW_BYTES)
        }),
      ),
          ...
          state.sawFrame
            ...
            : Stream.fail(
                eventError(
                  "Framing.sse",
                  state.raw.length > 0
                    ? "Provider returned HTTP 200 with a body that is not a valid SSE stream (no data events)"
                    : "Provider returned an empty HTTP 200 response",
                  state.raw.length > 0 ? state.raw : undefined,
                ),
              ),
        ),
      ),
