...
// Quota exhaustion is permanent for the window — retrying just delays the error
// the caller sees. Providers phrase it very differently: OpenAI's
// `insufficient_quota`, OpenCode Go's `GoUsageLimitError` ("5-hour usage limit
// reached"), Codex's usage-limit and billing messages, "no credits", etc. These
// all mean "stop and surface the error", unlike a plain rate limit.
const QUOTA_BODY = /insufficient[-_\s]?quota|quota[-_\s]?exceeded|usage[-_\s]?limit|usage[-_\s]?quota|goUsageLimitError|no (?:more )?(?:credits?|quota)|out of (?:credits?|quota)|billing/i
